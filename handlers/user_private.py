...
# @user_private_router.message((F.text.lower().contains("menu")) | (F.text.lower().contains("commands")))
...
@user_private_router.message(StateFilter(None), Command("enter_city_place"))
...
async def adding_city(message: types.Message, state: FSMContext):
    ...
    await state.update_data(city=city_name)
     # get_weather.get_geo(city=city_name)
...
# @user_private_router.message(AddCity.gettingForecast, F.text)
# async def adding_forecast(message: types.Message, state: FSMContext):
#     await state.update_data(forecast=message.text)
#     data = await state.get_data()
#     await message.answer(f"your data:\n{str(data)}")
#     await state.clear()

@user_private_router.message(AddCity.gettingForecast, F.text.lower() == "current weather")
@user_private_router.message(Command("current_weather"))
async def current_weather_cmd(message: types.Message, state: FSMContext):
    await state.update_data(forecast="current_weather")
    await message.answer("<i><b>Getting forecast...</b></i>")
    await state.update_data(forecast=message.text)
    user_data = await state.get_data()
    lati, long = get_weather.get_geo(city=user_data.get("city"))
    weather = get_weather.get_weather(lat=lati, lon=long)
    await message.answer(f"<i><b>{get_weather.format_weather(city=user_data.get("city"), data=weather)}</b></i>")
    # await state.clear()
    await message.answer("What are you going to see next?", reply_markup=reply.get_kbd(
